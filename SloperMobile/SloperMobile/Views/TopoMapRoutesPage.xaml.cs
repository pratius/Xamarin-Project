...
namespace SloperMobile.Views
{
    public partial class TopoMapRoutesPage : ContentPage
    {
        ...
        string routeIdData = string.Empty;       
        Double height, newHeight;
        int _routeId = 0;
        public TopoMapRoutesPage(MapListModel CurrentSector, string _lstData, int routeId, int routeIndex,List<RouteId> routeList)
        {
            try
            {
                InitializeComponent();
                ...
                _routeId = routeId;
               if(routeList != null)
                routeIdData = JsonConvert.SerializeObject(routeList);
                    ...
                    Device.BeginInvokeOnMainThread(() =>
                    {
                        ...
                        if (topoimg != null && topoimg.Count > 0)
                        {
                            if (!string.IsNullOrEmpty(topoimg[0].image.data))
                            {
                                ...
                                {
                                    webView.CallJsFunction("initReDrawing", staticAnnotationData, listData, newHeight, Convert.ToInt32(t), true, false, _bucket,routeIndex);
                                }
                            }
                            else
                            {
        ...
        private void OnLoadFinished(object sender, EventArgs args)
        {
            ...
            if (topoimg != null && topoimg.Count > 0)
            {
                if (!string.IsNullOrEmpty(topoimg[0].image.data))
                {
                    ...
                    {
                        webView.CallJsFunction("initDrawing", staticAnnotationData, listData, newHeight, _bucket,_routeIndex, routeIdData);
                    }
                }
                else
                {
                ...
                if (topoimgages.Count > 0 && topoimgages != null)
                {
                    if (topoimg != null && topoimg.Count > 0)
                    {
                        if (!string.IsNullOrEmpty(topoimg[0].image.data))
                        {
                            ...
                            {
                                webView.CallJsFunction("initReDrawing", staticAnnotationData, listData, (newHeight), _routeId, true, false, _bucket, _routeIndex);
                            }
                        }
                        else
                        {
