...
namespace SloperMobile.Views
{
    public partial class TopoMapRoutesPage : ContentPage
    {
        ...
        private async void OnScroll(object obj)
        {
            // await Task.Yield();
            // await scrollView.ScrollToAsync(Convert.ToDouble(obj), 0, true);
        }
        ...
        private void canvas_Pinch(UIKit.UIPinchGestureRecognizer arg1)
        {
            if (arg1.State == UIKit.UIGestureRecognizerState.Began)
            {
            }
            if (arg1.State == UIKit.UIGestureRecognizerState.Changed)
            {
                float minScale = 1.0f;
                float maxScale = 3.0f;
                float zoomSpeed = .5f;

                float deltaScale = (float)arg1.Scale;

                deltaScale = ((deltaScale - 1) * zoomSpeed) + 1;

                deltaScale = Math.Min(deltaScale, maxScale / _currentScale);
                if (deltaScale > minScale)
                    deltaScale = Math.Max(deltaScale, minScale / _currentScale);

                _currentScale = deltaScale;


                arg1.View.Transform *= CGAffineTransform.MakeScale(deltaScale, deltaScale);

                // Reset the gesture recognizer's scale - the next callback will get a delta from the current scale.
                arg1.Scale = 1;
            }
            if (arg1.State == UIKit.UIGestureRecognizerState.Ended)
            {
        ...
        public void DrawLine(SKCanvas _skCanvas, int route, float ratio, int _height, int _width)
        {
            ...
            using (var path = new SKPath())
            {
                        ...
                        for (int k = 0; k < topoimg[0].drawing[j].line.points.Count; k++)
                        {
                            ...
                            else
                            {
                                if (globalWidth > globalHeight)
                                {
                                    ...
                                    if (globalWidth / 2 < xVal)
                                    {
                                        // scrollView.ScrollToAsync(topoimg[0].drawing[j].line.points.Count > 0 ? Convert.ToDouble((float.Parse(topoimg[0].drawing[j].line.points[0].x) * ratio / 6)) : 0, 0, true);
                                    }
                                }
                            }
                        }
                ...
                if (Device.OS == TargetPlatform.Android)
                {
                    canvas.HeightRequest = _height;
                    canvas.WidthRequest = (_width / 4) + 80;
                }
                else
                {
                    canvas.HeightRequest = _height;
                    canvas.WidthRequest = (_width / 2) - 2;
                }
                path.Close();
            }
        }
        ...
        public void DrawAnnotation(topoline topoimg, SKCanvas _skCanvas, float ratio, string gradeBucket, int _routecnt)
        {
            for (int i = 0; i < topoimg.points.Count; i++)
            {
                ...
                {
                    if (topoimg.style.is_dark_checked != null)
                    {
